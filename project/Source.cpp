...
static int animationPeriod = 4; // milliseconds
...
void keyInput(unsigned char key, int x, int y) {
	switch (key) {
		case 27: // ASCII for Escs
			exit(0);
		case ' ':
			if (isAnimate) // game is playing
				isAnimate = 0; // pause game
			else { // not playing
				isAnimate = 1; // start
				animate(1);
			}
